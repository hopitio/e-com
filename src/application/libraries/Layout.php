...
class Layout
{
	...
	function view($view, $data=null, $return=false)
	{
		...
		// CSS適用
		if (!isset($loadedData["css"]) ) $loadedData["css"] = array();
		$loadedData["css"] = array_merge($loadedData["css"], $this->css);
		
		// Javascript適用
		if (!isset($loadedData["javascript"]) ) $loadedData["javascript"] = array();
		$loadedData["javascript"] = array_merge($loadedData["javascript"], $this->javascript);
		
		// Metatag適用
		if (!isset($loadedData["meta"]) ) $loadedData["meta"] = array();
	 ...
	 * 
	 * @param string javascript_file 例：
	 */
	 ...
	 * メタタグを追加する
	 * 
	 * @param array meta tag 例：array('name' => 'robots', 'content' => 'no-cache')
	 */
	function meta($meta) {
		$this->meta[] = $meta;
	}
