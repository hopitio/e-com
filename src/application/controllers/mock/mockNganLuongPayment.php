...
class mockNganLuongPayment extends BaseController{
    ...
    private function callSevice($param){
        $protocol = strtolower(substr($_SERVER["SERVER_PROTOCOL"],0,strpos( $_SERVER["SERVER_PROTOCOL"],'/'))).'://';
        $client = new nusoap_client($protocol.$_SERVER['HTTP_HOST'].'/portal/order/nganluong_sync'.'?wsdl',true);
        $response = $client->call('UpdateOrder',$param);
    }
